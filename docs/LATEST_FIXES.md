...
### السبب الجذري
استراتيجية تقسيم chunks في `vite.config.ts` كانت تفصل React إلى chunk منفصل (`react-core`) بينما `react-is` يذهب إلى `vendor`. هذا يسبب تحميل `vendor` (الذي يحتوي مكتبات تعتمد على React) قبل `react-core`.

### التحليل التقني
```javascript
// ❌ قبل: react-core chunk منفصل
if (id.match(/node_modules[\\/](react|react-dom|scheduler)[\\/]/) && 
    !id.includes('react-is')) {  // react-is يذهب لـ vendor!
  return 'react-core';
}

// المشكلة: next-themes في react-ui-libs يحتاج react-is
// لكن react-is في vendor يُحمّل قبل react-core
```

### الحل المنفذ
```javascript
// ✅ بعد: إزالة react-core كـ chunk منفصل
// React و react-dom و react-is كلهم في vendor
...
| الملف | التغيير |
|-------|---------|
| `vite.config.ts` | إزالة react-core chunk، تبسيط الاستراتيجية |
| `src/lib/version.ts` | تحديث الإصدار إلى 2.6.7 |
| `VERSION` | تحديث الإصدار إلى 2.6.7 |

### النتائج
- ✅ صفحة الترحيب تعمل بدون أخطاء
- ✅ ترتيب تحميل المكتبات صحيح
- ✅ chunks أقل تعقيداً وأكثر موثوقية
...
| الفئة | v2.6.0 | v2.6.4 | v2.6.5 | v2.6.6 | v2.6.7 |
|-------|--------|--------|--------|--------|--------|
| LCP | - | - | <2.5s | **< 0.5s** | < 0.5s |
| Dashboard Load | 3.3s | **1.1s** | 1.1s | 1.1s | 1.1s |
| Hooks تنظيم | - | **152 في 18 مجلد** | - | - | - |
| RLS Policies | **مُبسطة** | - | - | - | - |
| Service Worker | **مُصلح** | - | - | - | - |
| Auth للصفحات العامة | - | - | - | **فوري** | فوري |
| Vite Chunks | - | - | - | - | **مُبسطة** |
