...
### 1. إصلاحات الأمان (RLS Policies) ✅

| السياسة | الإصلاح |
|---------|---------|
| `contact_messages` | تقييد القراءة للإداريين فقط |
| `contracts` | إزالة وصول المستفيدين لجميع العقود |
| `rental_payments` | تقييد للموظفين الماليين |
| `invoices` | المستفيدون يرون فواتيرهم فقط |
