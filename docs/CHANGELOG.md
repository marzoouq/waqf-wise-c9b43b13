...
**الإصدار الحالي:** 3.1.0 | **آخر تحديث:** 2025-12-24
...
- إضافة قسم **"أدوات المطور"** في القائمة الجانبية مع 7 صفحات:
  ...
  - صحة قاعدة البيانات (`/db-health`)
  - أداء قاعدة البيانات (`/db-performance`)
  - إدارة التكاملات (`/integrations`)
  - اختبار Edge (`/edge-test`)
- إضافة صفحة **المساعدات الطارئة** (`/emergency-aid`) في قسم إدارة الوقف
- إضافة **تقرير أعمار المستأجرين** (`/tenants/aging-report`) في القسم المالي
- إضافة صفحات إعدادات جديدة:
  - إعدادات الإشعارات (`/notifications/settings`)
  - إعدادات الصفحة الرئيسية (`/settings/landing-page`)
...
- **تصحيح مسار قاعدة المعرفة** من `/knowledge` إلى `/knowledge-base`
