                      ...
                      return (
                        <Card key={budget.id} className="p-4">
                          ...
                          {budget.variance_amount !== null && (
                            <div
                              className={`text-sm mb-3 ${getVarianceColor(budget.variance_amount)}`}
                            >
                              الانحراف: {formatCurrency(Math.abs(budget.variance_amount || 0))}
                              <Badge
                                variant={budget.variance_amount >= 0 ? 'default' : 'destructive'}
                        ...
                        {budgets.map((budget) => {
                          ...
                          return (
                            <TableRow key={budget.id}>
                              ...
                              <TableCell
                                className={`hidden lg:table-cell text-xs sm:text-sm ${getVarianceColor(budget.variance_amount)}`}
                              >
                                {formatCurrency(Math.abs(budget.variance_amount || 0))}
                                {budget.variance_amount !== null && (
                                  <Badge
                                    variant={
                                      budget.variance_amount >= 0 ? 'default' : 'destructive'
                                    }
                                    className="me-2 text-xs"
                                  >
                                    {budget.variance_amount >= 0 ? 'توفير' : 'تجاوز'}
                              ...
                              <TableCell>
                                <div className="flex gap-1">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      confirmDelete(budget.id, budget.accounts?.name_ar)
                                    }
                                  >
