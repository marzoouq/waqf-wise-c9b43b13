...
const Properties = () => {
  ...
  const handleAddClick = useCallback(
    (type: 'property' | 'contract' | 'payment' | 'maintenance') => {
      const handlers = {
        ...
        {!statsLoading && stats && (
          <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-6">
            ...
            <Card className={stats.expiringContracts > 0 ? 'border-destructive' : ''}>
              ...
              <CardContent className="p-3 sm:p-6 pt-0">
                <div
                  className={`text-xl sm:text-2xl font-bold ${stats.expiringContracts > 0 ? 'text-destructive' : ''}`}
                >
                  {stats.expiringContracts}
                </div>
                <p className="text-xs text-muted-foreground mt-1">خلال 30 يوم القادمة</p>
              </CardContent>
            </Card>
          </div>
        )}
