...
export default function BeneficiaryAccountStatement() {
  ...
  const { beneficiary, payments, isLoading, error, refetch, calculateStats } =
    useBeneficiaryAccountStatementData(user?.id, { dateFrom, dateTo, paymentMethod });
  ...
  const exportToPDF = async () => {
    ...
    yPos += 7;
    doc.text(`التاريخ: ${format(new Date(), 'dd/MM/yyyy', { locale: ar })}`, pageWidth - 20, yPos, {
      align: 'right',
    });
    yPos += 10;
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(
      `إجمالي المدفوعات: ${formatCurrencyForPDF(stats.totalPayments)}`,
      pageWidth - 20,
      yPos,
      { align: 'right' }
    );
    ...
    yPos += 6;
    doc.text(`متوسط الدفعة: ${formatCurrencyForPDF(stats.avgPayment)}`, pageWidth - 20, yPos, {
      align: 'right',
    });
    yPos += 10;
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-success">
                  {formatCurrency(stats.largestPayment)}
                </div>
              </CardContent>
            </Card>
          </div>
