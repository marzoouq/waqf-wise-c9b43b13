...
export default function FiscalYearsManagement() {
  ...
  return (
    <MobileOptimizedLayout>
      <div className="mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <Button onClick={() => setAddYearOpen(true)} className="gap-2">
              <Plus className="h-4 w-4" />
              إضافة سنة مالية
            </Button>
            <Button variant="outline" onClick={() => setShowTests(!showTests)} className="gap-2">
      ...
      <div className="space-y-6">
        ...
        {activeYear && (
          <Alert>
            ...
            <AlertDescription>
              السنة المالية النشطة حالياً: <strong>{activeYear.name}</strong> من{' '}
              {format(new Date(activeYear.start_date), 'dd MMMM yyyy', { locale: ar })} إلى{' '}
              {format(new Date(activeYear.end_date), 'dd MMMM yyyy', { locale: ar })}
            </AlertDescription>
          </Alert>
        )}
        ...
        {selectedYear && (
          <Card>
            ...
            <CardContent>
              <FiscalYearSummaryCard fiscalYearId={selectedYear.id} closing={selectedClosing} />
            </CardContent>
          </Card>
        )}
        ...
        {!selectedYear && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-lg text-muted-foreground mb-2">اختر سنة مالية لعرض تفاصيلها</p>
      ...
      <AddFiscalYearDialog open={addYearOpen} onOpenChange={setAddYearOpen} />
    </MobileOptimizedLayout>
  );
}
