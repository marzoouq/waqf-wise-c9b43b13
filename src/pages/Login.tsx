    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (msg.includes('invalid_credentials') || msg.includes('invalid login credentials')) {
          errorTitle =
            loginType === 'beneficiary' ? 'بيانات الدخول غير صحيحة' : 'كلمة المرور غير صحيحة';
          errorMessage =
      ...
      <main>
        <Card className="w-full max-w-md">
          <CardHeader className="space-y-1 text-center">
            ...
            <CardDescription>اختر نوع الحساب وأدخل بياناتك للدخول إلى النظام</CardDescription>
          </CardHeader>

          <Tabs
            value={loginType}
            onValueChange={(v) => setLoginType(v as 'staff' | 'beneficiary')}
            className="px-6"
          >
          ...
          <div className="text-center text-sm text-muted-foreground px-6 pb-4 space-y-2">
            <Link
              to="/tenant-portal"
              className="text-primary hover:underline inline-flex items-center justify-center gap-1 w-full"
            >
            ...
            <Link
              to="/install"
              className="text-muted-foreground hover:text-primary hover:underline inline-flex items-center justify-center gap-1 w-full"
            >
