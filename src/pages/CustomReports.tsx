  ...
  const columns: Column<ReportTemplate>[] = [
    ...
    },
    {
      ...
      render: (value: boolean) => (
        <Badge variant={value ? 'default' : 'secondary'}>{value ? 'عام' : 'خاص'}</Badge>
      ),
    },
    {
      key: 'created_at',
      ...
      hideOnMobile: true,
      render: (value: string) =>
        value ? format(new Date(value), 'dd/MM/yyyy', { locale: ar }) : '-',
    },
    {
      key: 'actions',
      label: 'الإجراءات',
      render: (_: unknown, row: ReportTemplate) => (
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" title="عرض التقرير" onClick={() => handleView(row)}>
          ...
          >
            <Star
              className={`h-4 w-4 ${row.is_favorite ? 'fill-yellow-400 text-yellow-400' : ''}`}
            />
          </Button>
          <Button variant="ghost" size="icon" title="حذف" onClick={() => handleDelete(row.id)}>
