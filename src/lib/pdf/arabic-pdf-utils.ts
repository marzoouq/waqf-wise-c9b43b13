...
function processRTLMixedText(text: string): string {
  ...
  // إصلاح الأرقام مع العملة والرموز المرتبطة:
  // - أرقام عربية وغربية
  // - الفواصل والنقاط (1,234,567.89)
  // - النسب المئوية (15%)
  // - الوحدات النقدية (ر.س)
  // - التواريخ (2024/01/15 أو 15-01-2024)
  rtl = rtl.replace(
    /[0-9\u0660-\u0669][0-9\u0660-\u0669,\.\u066B\u066C\/%:\-\/]*(\s*[رس]\.[رس])?/g,
    (m) => reverse(m)
  );

  // إصلاح "ر.س" المنفصلة التي قد تظهر بعد الأرقام
