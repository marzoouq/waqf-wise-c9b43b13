...
export async function generateDistributionPDF(
  ...
  if (distribution.approved_at) {
    distributionInfo.push({
      ...
      value: distribution.approved_at.split('T')[0],
    });
  }
  ...
  for (const info of distributionInfo) {
    ...
    pdf.text(processArabicText(`${info.label}:`), pageWidth - margin, yPosition, {
      align: 'right',
    });
  ...
  if (distribution.notes) {
    ...
    const noteLines = pdf.splitTextToSize(
      processArabicText(distribution.notes),
      pageWidth - margin * 2
    );
  ...
  if (vouchers && vouchers.length > 0) {
    ...
    pdf.setFontSize(12);
    pdf.setTextColor(...WAQF_COLORS.primary);
    pdf.text(processArabicText('تفاصيل التوزيع'), pageWidth - margin, yPosition, {
      align: 'right',
    });
    yPosition += 8;
    ...
    pdf.text(processArabicText('اسم المستفيد'), pageWidth - margin - 20, yPosition + 5.5, {
      align: 'right',
    });
    ...
    for (let i = 0; i < vouchers.length; i++) {
      ...
      pdf.text(processArabicText(String(i + 1)), pageWidth - margin - 5, yPosition + 5, {
        align: 'right',
      });
      pdf.text(
        processArabicText(voucher.beneficiary_name),
        pageWidth - margin - 20,
        yPosition + 5,
        { align: 'right' }
      );
      pdf.text(
        processArabicText(formatCurrencyForPDF(voucher.amount)),
        margin + 60,
        yPosition + 5,
        { align: 'right' }
      );
      pdf.text(processArabicText(getStatusLabel(voucher.status)), margin + 20, yPosition + 5, {
        align: 'right',
      });
