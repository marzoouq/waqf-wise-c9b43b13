  ...
  try {
    ...
    if (invoice.qr_code_data) {
      try {
        ...
        doc.setFont('Amiri', 'bold');
        doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
        doc.text(processArabicText('رمز التحقق ZATCA'), qrBoxX + qrBoxWidth / 2, qrBoxY + 6, {
          align: 'center',
        });
    ...
    if (invoice.notes) {
      ...
      const noteLines = doc.splitTextToSize(
        processArabicText(invoice.notes),
    ...
    if (orgSettings) {
      ...
      doc.setFontSize(9);
      doc.text(processArabicText(orgSettings.organization_name_ar), pageWidth / 2, footerY + 9, {
        align: 'center',
      });

      // السطر الثاني - البريد والهاتف
      doc.setFontSize(8);
      const contactInfo = [orgSettings.phone || '', orgSettings.email || '']
        .filter(Boolean)
        .join(' | ');

      if (contactInfo) {
        doc.text(contactInfo, pageWidth / 2, footerY + 14, { align: 'center' });
      }
