...
export const BENEFICIARY_KEYS = {
  ...
  MY_BENEFICIARY: (userId?: string) =>
    userId ? (['my-beneficiary', userId] as const) : (['my-beneficiary'] as const),
  BENEFICIARY_PAYMENTS: (beneficiaryId: string, ...filters: string[]) =>
    ['beneficiary-payments', beneficiaryId, ...filters] as const,
  BENEFICIARY_ACTIVITY_LOG: (beneficiaryId?: string) =>
    ['beneficiary-activity-log', beneficiaryId] as const,
  BENEFICIARY_CATEGORIES: ['beneficiary-categories'] as const,
  BENEFICIARY_HEIR_DISTRIBUTIONS: (beneficiaryId: string) =>
    ['beneficiary-heir-distributions', beneficiaryId] as const,
  BENEFICIARY_EMERGENCY_AID: (beneficiaryId?: string) =>
    ['beneficiary-emergency-aid', beneficiaryId] as const,
  ...
  BENEFICIARY_STATISTICS: (beneficiaryId?: string) =>
    ['beneficiary-statistics', beneficiaryId] as const,
  BENEFICIARY_PROFILE: (userId?: string) => ['beneficiary-profile', userId] as const,
  BENEFICIARY_DOCUMENTS: (beneficiaryId?: string) =>
    ['beneficiary-documents', beneficiaryId] as const,
  BENEFICIARY_PROFILE_PAYMENTS: (beneficiaryId?: string) =>
    ['beneficiary-payments', beneficiaryId] as const,
  // ✅ تم تغيير المفتاح ليكون مختلفاً (كان يتشابه مع BENEFICIARY_REQUESTS)
  BENEFICIARY_PORTAL_REQUESTS: (beneficiaryId?: string) =>
    ['beneficiary-portal-requests', beneficiaryId] as const,
  FAMILY_TREE: (beneficiaryId: string) => ['family-tree', beneficiaryId] as const,
  BENEFICIARY_INTEGRATION_STATS: (beneficiaryId: string) =>
    ['beneficiary-integration-stats', beneficiaryId] as const,
  HEIR_DISTRIBUTIONS_SUMMARY: (beneficiaryId: string) =>
    ['heir-distributions-summary', beneficiaryId] as const,

  // Beneficiary Properties
  PROPERTIES_FOR_BENEFICIARY: ['properties-for-beneficiary'] as const,
  CONTRACTS_FOR_BENEFICIARY: (isPublished: boolean) =>
    ['contracts-for-beneficiary', isPublished] as const,
  ...
  BENEFICIARY_YEARLY_DISTRIBUTIONS: (beneficiaryId?: string, year?: number) =>
    ['beneficiary-yearly-distributions', beneficiaryId, year] as const,
  BENEFICIARY_YEARLY_REQUESTS: (beneficiaryId?: string, year?: number) =>
    ['beneficiary-yearly-requests', beneficiaryId, year] as const,
  ...
  // Eligibility
  ELIGIBILITY_ASSESSMENTS: (beneficiaryId: string) =>
    ['eligibility-assessments', beneficiaryId] as const,
