...
export class ArchiveService {
  ...
  static async createFolder(folder: FolderInsert): Promise<Folder> {
    const { data, error } = await supabase.from('folders').insert(folder).select().maybeSingle();
  ...
  static async getStats(): Promise<ArchiveStats> {
    ...
    const recentUploads = documents.filter((d) => new Date(d.created_at) > weekAgo).length;

    // Count this month additions
    const startOfMonth = new Date();
    startOfMonth.setDate(1);
    startOfMonth.setHours(0, 0, 0, 0);
    const thisMonthAdditions = documents.filter(
    ...
    const formatBytes = (b: number) =>
      b === 0
        ? '0 B'
        : `${(b / Math.pow(1024, Math.floor(Math.log(b) / Math.log(1024)))).toFixed(1)} ${['B', 'KB', 'MB', 'GB'][Math.floor(Math.log(b) / Math.log(1024))]}`;
  ...
  static async getArchivistStats(): Promise<{
    ...
    const todayUploads =
      allDocsRes.data?.filter((doc) => doc.uploaded_at?.startsWith(today)).length || 0;
  ...
  static async getDocumentTags(documentId?: string) {
    let query = supabase
      .from('document_tags')
      .select('id, document_id, tag_name, tag_type, confidence_score, created_at, created_by');
   ...
   * إضافة وسم لمستند
   */
  static async addDocumentTag(tag: {
    document_id: string;
    tag_name: string;
  ...
  static async smartSearch(query: string, searchType: 'text' | 'tags' | 'ocr') {
    if (searchType === 'ocr') {
      ...
      const { data, error } = await supabase
        .from('documents')
        .select(
          'id, name, description, file_type, file_path, file_size, folder_id, created_at, updated_at'
        )
  ...
  static async getDocumentVersions(documentId: string) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'id, document_id, version_number, file_path, file_size, change_description, created_by, created_at, is_current, metadata'
      )
  ...
  > {
    const { data, error } = await supabase.from('documents').select('category, file_size');
