...
export const FinancialCardsService = {
   ...
   */
  async getRevenueProgress(fiscalYear: {
    id: string;
    start_date: string;
    end_date: string;
  }): Promise<{
    ...
    const [paymentsResult, vouchersResult, contractsResult, revenueEntriesResult] =
      await Promise.all([
    ...
    const rentalCollected = payments.reduce(
      (sum, p) => sum + (p.amount_paid || p.amount_due || 0),
      0
    );
  ...
  async getWaqfSummary() {
    const { data: publicStats, error: publicError } = await supabase.rpc('get_waqf_public_stats');
