...
export class UnifiedFinancialService {
  ...
  private static async getRevenueDataFallback(): Promise<UnifiedRevenueData> {
    try {
      const [
        ...
        supabase.from('rental_payments').select('amount_paid').eq('status', 'paid'),
        ...
        supabase.from('contracts').select('monthly_rent').eq('status', 'active'),
        ...
        supabase.from('rental_payments').select('amount_due').in('status', ['pending', 'overdue']),
      ]);

      const rentalCollected =
        rentalPaymentsRes.data?.reduce((sum, p) => sum + (p.amount_paid || 0), 0) || 0;

      const vouchersCollected =
        receiptVouchersRes.data?.reduce((sum, v) => sum + (v.amount || 0), 0) || 0;

      const totalCollected = rentalCollected + vouchersCollected;

      const monthlyContractRevenue =
        activeContractsRes.data?.reduce((sum, c) => sum + (c.monthly_rent || 0), 0) || 0;

      const expectedAnnualRevenue = monthlyContractRevenue * 12;

      const accountingPeriodRevenue =
        revenueAccountsRes.data?.reduce((sum, a) => sum + (a.current_balance || 0), 0) || 0;

      const pendingAmount =
        pendingPaymentsRes.data?.reduce((sum, p) => sum + (p.amount_due || 0), 0) || 0;
  ...
  static async getDistributionData(): Promise<UnifiedDistributionData> {
    try {
      const [distributionsRes, heirDistributionsRes] = await Promise.all([
        ...
        supabase.from('heir_distributions').select('share_amount'),
      ]);
      ...
      const beneficiaryDistributions =
        heirDistributionsRes.data?.reduce((sum, h) => sum + (h.share_amount || 0), 0) || 0;
  ...
  static async getIncomeData(): Promise<UnifiedIncomeData> {
    try {
      const [revenueAccountsRes, expenseAccountsRes, paymentVouchersRes, disclosuresRes] =
        await Promise.all([
          supabase
            .from('accounts')
            .select('current_balance')
      ...
      const accountExpenses =
        expenseAccountsRes.data?.reduce((sum, a) => sum + (a.current_balance || 0), 0) || 0;

      const voucherExpenses =
        paymentVouchersRes.data?.reduce((sum, v) => sum + (v.amount || 0), 0) || 0;
  ...
  static async getPropertyCollectionSummary(propertyId?: string) {
    try {
      const { data, error } = await supabase.rpc('get_property_collection_summary', {
  ...
  static async getTotalCollected(startDate?: string, endDate?: string): Promise<number> {
    try {
      const { data, error } = await supabase.rpc('get_total_collected', {
  ...
  static async getCollectionPercentage(): Promise<number> {
    try {
      const { data, error } = await supabase.rpc('get_collection_percentage');
