...
export class AuthService {
  ...
  static async logout(options?: {
    ...
    const keysToKeep =
      options?.keepTheme !== false ? ['theme', 'vite-ui-theme', 'language', 'i18nextLng'] : [];
  ...
  static async getUserProfile(userId: string): Promise<UserProfile | null> {
    const [profileRes, rolesRes] = await Promise.all([
      supabase
        ...
        .eq('user_id', userId)
        .maybeSingle(),
      supabase.from('user_roles').select('role').eq('user_id', userId).is('deleted_at', null), // استبعاد السجلات المحذوفة
    ]);
   ...
   */
  static async getBeneficiaryEmailByNationalId(
    nationalId: string
  ): Promise<BeneficiaryEmailResult | null> {
    const { data, error } = await supabase.rpc('get_beneficiary_email_by_national_id', {
