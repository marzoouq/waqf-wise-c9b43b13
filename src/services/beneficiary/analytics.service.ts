...
export class BeneficiaryAnalyticsService {
   ...
   * جلب سجل المدفوعات
   */
  static async getPaymentsHistory(
   ...
   */
  static async getEmergencyAid(
    beneficiaryId: string
  ): Promise<Database['public']['Tables']['emergency_aid']['Row'][]> {
    try {
      const { data, error } = await supabase
        .from('emergency_aid')
        .select('*')
  ...
  static async getStatisticsRPC(beneficiaryId: string) {
    try {
      const { data, error } = await supabase.rpc('get_beneficiary_statistics', {
  ...
  > {
    ...
    requestsRes.data?.forEach((r: RequestWithType) =>
      timelineEvents.push({
        id: r.id,
        type: 'request',
