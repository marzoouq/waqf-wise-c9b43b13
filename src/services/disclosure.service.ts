...
export class DisclosureService {
  ...
  static async getDisclosureBeneficiaries(disclosureId: string) {
    const { data, error } = await supabase
      .from('disclosure_beneficiaries')
      .select(
        'id, disclosure_id, beneficiary_id, beneficiary_name, beneficiary_type, relationship, allocated_amount, payments_count, created_at'
      )
      .eq('disclosure_id', disclosureId)
