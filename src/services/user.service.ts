...
export class UserService {
  ...
  static async getActiveSessions(userId: string) {
    const { data, error } = await supabase
      .from('user_sessions')
      .select(
        'id, user_id, session_token, ip_address, user_agent, device_info, is_active, last_activity_at, created_at'
      )
      .eq('user_id', userId)
      .eq('is_active', true)
  ...
  static async getAvailableUsers() {
    const { data, error } = await supabase.from('user_roles').select('user_id, role').limit(200);
