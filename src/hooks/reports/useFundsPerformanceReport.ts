...
export function useFundsPerformanceReport() {
    ...
    queryFn: async () => {
      ...
      return (funds || [])
        ...
        .map((fund) => ({
          ...
          utilization_rate:
            Number(fund.allocated_amount) > 0
              ? (Number(fund.spent_amount) / Number(fund.allocated_amount)) * 100
              : 0,
          avg_per_beneficiary:
            fund.beneficiaries_count > 0 ? Number(fund.spent_amount) / fund.beneficiaries_count : 0,
        }))
  ...
  const { data: categoryDistribution } = useQuery({
    ...
    queryFn: async (): Promise<CategoryDistribution[]> => {
      ...
      const categories = fundPerformance.reduce(
        (acc, fund) => {
