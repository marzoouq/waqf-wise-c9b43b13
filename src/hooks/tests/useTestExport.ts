  ...
  const exportToPDF = useCallback(
    async (
      ...
      try {
        ...
        doc.setFontSize(11);
        const summaryY = 55;
        doc.text(`إجمالي الاختبارات: ${results.length}`, pageWidth - 15, summaryY, {
          align: 'right',
        });
        doc.text(`ناجح: ${passed} | فاشل: ${failed}`, pageWidth - 15, summaryY + 7, {
          align: 'right',
        });
        doc.text(`نسبة النجاح: ${passRate}%`, pageWidth - 15, summaryY + 14, { align: 'right' });
        doc.text(`متوسط الزمن: ${avgDuration}ms`, pageWidth - 15, summaryY + 21, {
          align: 'right',
        });
        ...
        if (failedTests.length > 0) {
          ...
          failedTests.slice(0, 20).forEach((test, index) => {
            ...
            doc.text(`   المشكلة: ${test.message || 'غير محدد'}`, pageWidth - 20, y + 6, {
              align: 'right',
            });
            y += 15;
          });
    ...
    async (
      ...
      try {
        ...
        summarySheet.addRows([
          ...
          {
            item: 'متوسط الزمن',
            value: `${Math.round(results.reduce((s, r) => s + r.duration, 0) / results.length)}ms`,
          },
        ]);
