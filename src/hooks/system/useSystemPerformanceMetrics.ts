...
export function useSystemPerformanceMetrics() {
  return useQuery({
    ...
    queryFn: async (): Promise<PerformanceDataPoint[]> => {
      ...
      const hours = eachHourOfInterval({ start: yesterday, end: now }).filter(
        (_, index) => index % DASHBOARD_METRICS.PERFORMANCE_HOURS_INTERVAL === 0
      );
      ...
      const dataPoints: PerformanceDataPoint[] = hours.map((hour, index) => {
        ...
        const avgResponseTime =
          periodHealth.length > 0
            ? Math.round(
                periodHealth.reduce((sum, h) => sum + (h.response_time_ms || 0), 0) /
                  periodHealth.length
              )
            : 0;

        const avgCpu =
          periodHealth.length > 0 ? Math.min(100, Math.round(avgResponseTime / 10)) : 0;
