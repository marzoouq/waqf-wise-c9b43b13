  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CollectionStats> => {
      ...
      const [
        ...
        // دفعات الإيجار
        supabase.from('rental_payments').select('id, amount_due, amount_paid, status, due_date'),
        ...
        supabase
          .from('contracts')
          .select(
            'id, monthly_rent, payment_frequency, status, end_date, tenant_name, tenant_id, property_id'
          )
          .eq('status', 'نشط'),
      ...
      const overduePayments = rentalPayments.filter(
        (p) =>
