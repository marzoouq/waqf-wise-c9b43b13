...
const INVALIDATION_MAP: Record<string, readonly (readonly string[])[]> = {
  ...
  accounts: [['accounts'] as const, ['chart-of-accounts'] as const, ['trial-balance'] as const],
  bank_accounts: [['bank-accounts'] as const, ['bank-balance'] as const],
  bank_transactions: [['bank-transactions'] as const, ['bank-reconciliation'] as const],
  payments: [['payments'] as const, ['unified-dashboard-kpis'] as const],
  rental_payments: [
    ['rental-payments'] as const,
    ['revenue-progress'] as const,
    ['unified-dashboard-kpis'] as const,
  ],
  fiscal_years: [['fiscal-year'] as const, ['unified-dashboard-kpis'] as const],
  fiscal_year_closings: [['fiscal-year-closings'] as const, ['waqf-corpus'] as const],
};
 ...
 */
export function useAccountantDashboardRealtime(
  options: UseAccountantDashboardRealtimeOptions = {}
) {
