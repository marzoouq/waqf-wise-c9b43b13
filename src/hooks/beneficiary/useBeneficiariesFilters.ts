...
export function useBeneficiariesFilters(
  ...
  const filteredBeneficiaries = useMemo(() => {
    ...
    if (advancedCriteria.fullName) {
      results = safeFilter(results, (b) =>
        b.full_name.toLowerCase().includes(advancedCriteria.fullName!.toLowerCase())
      );
    }
    ...
    if (advancedCriteria.tribe) {
      results = safeFilter(
        results,
        (b) => b.tribe && b.tribe.toLowerCase().includes(advancedCriteria.tribe!.toLowerCase())
      );
    }
    if (advancedCriteria.city) {
      results = safeFilter(
        results,
        (b) => b.city && b.city.toLowerCase().includes(advancedCriteria.city!.toLowerCase())
      );
    }
    ...
    if (advancedCriteria.priorityLevel) {
      results = safeFilter(
        results,
        (b) => String(b.priority_level || 1) === advancedCriteria.priorityLevel
      );
    }
  ...
  const stats = useMemo(() => {
    ...
    const suspendedBeneficiaries = safeFilter(beneficiaries, (b) =>
      matchesStatus(b.status, 'suspended')
    );
