...
export function useBeneficiaryAccountStatementData(userId: string | undefined, filters: PaymentFilters) {
  const { data: beneficiary, isLoading: beneficiaryLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("beneficiaries").select("id, full_name, beneficiary_number, account_balance, total_received").eq("user_id", userId).single();
  ...
  const { data: payments = [], isLoading: paymentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase.from("payments").select("id, amount, payment_date, description, payment_method").eq("beneficiary_id", beneficiary.id).order("payment_date", { ascending: false });
