...
export function InternalMessagesDialog({ open, onOpenChange }: InternalMessagesDialogProps) {
  const { user } = useAuth();
  const { inboxMessages, sentMessages, sendMessage, markAsRead, unreadCount } =
    useInternalMessages();
            ...
            <Card className="bg-muted/30">
              <CardContent className="p-3">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  <p>
                    <strong>الموضوع:</strong> {replyToMessage.subject}
                  </p>
                  <p className="line-clamp-2">
                    <strong>الرسالة الأصلية:</strong> {replyToMessage.body}
                  </p>
                </div>
                <Button
                  size="sm"
