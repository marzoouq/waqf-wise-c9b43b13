...
export function UnifiedKPICard({
  ...
  return (
    ...
    >
      <CardHeader
        className={cn('flex flex-row items-center justify-between space-y-0', sizeStyle.header)}
      >
        <CardTitle
          className={cn('font-medium text-muted-foreground truncate max-w-[80%]', sizeStyle.title)}
        >
          {title}
        </CardTitle>
        <Icon
          className={cn('transition-transform flex-shrink-0', sizeStyle.icon, variantStyle.icon)}
        />
      </CardHeader>
      <CardContent className={sizeStyle.content}>
        <div className={cn('mb-0.5 sm:mb-1 truncate', sizeStyle.value, variantStyle.text)}>
          {value}
        </div>
        {subtitle && (
          <p className={cn('text-muted-foreground truncate', sizeStyle.subtitle)}>{subtitle}</p>
        )}
        {parsedTrend && (
