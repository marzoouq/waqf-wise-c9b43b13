...
export function BeneficiaryReports() {
  ...
  const { beneficiaries, isLoading, isRefetching, lastUpdated, handleRefresh, error } =
  ...
  const columns: Column<BeneficiaryReportData>[] = [
    ...
    },
    {
      key: 'status',
      label: 'الحالة',
      render: (value) => <Badge variant={value === 'نشط' ? 'default' : 'secondary'}>{value}</Badge>,
    },
    {
      key: 'city',
      label: 'المدينة',
  ...
  if (error) {
    return (
      <ErrorState
        title="خطأ في تحميل بيانات المستفيدين"
