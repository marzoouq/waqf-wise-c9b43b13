...
export function AnnualDisclosuresReport() {
  ...
  const { data: disclosures, isLoading, error, refetch } = useQuery({
  ...
  const handleDownloadPDF = async (disclosure: AnnualDisclosureDetailed) => {
    ...
    try {
      ...
      await generateDisclosurePDF(disclosure as unknown as Parameters<typeof generateDisclosurePDF>[0], beneficiaries || [], null);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-success">{formatCurrency(disclosures.reduce((sum, d) => sum + d.total_revenues, 0))} ر.س</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-destructive">{formatCurrency(disclosures.reduce((sum, d) => sum + d.total_expenses, 0))} ر.س</p>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-chart-1">{formatCurrency(disclosures.reduce((sum, d) => sum + d.net_income, 0))} ر.س</p>
      ...
      <Accordion type="single" collapsible className="space-y-4">
        {disclosures.map((disclosure, index) => {
          ...
          return (
            <AccordionItem key={disclosure.id} value={disclosure.id} className="border rounded-lg overflow-hidden">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3 bg-success/10 rounded-lg text-center">
                      ...
                      <p className="text-sm font-bold text-success">{formatCurrency(disclosure.total_revenues)} ر.س</p>
                    </div>
                    <div className="p-3 bg-destructive/10 rounded-lg text-center">
                      ...
                      <p className="text-sm font-bold text-destructive">{formatCurrency(disclosure.total_expenses)} ر.س</p>
                    </div>
                    <div className="p-3 bg-primary/10 rounded-lg text-center">
                      <Wallet className="h-4 w-4 text-primary mx-auto mb-1" />
                      <p className="text-xs text-muted-foreground">الصافي</p>
                      <p className="text-sm font-bold text-primary">{formatCurrency(disclosure.net_income)} ر.س</p>
                    </div>
                  </div>
                    ...
                    <CardContent className="py-0 pb-3">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                        <div className="p-2 bg-muted/50 rounded">
                          ...
                          <p className="font-semibold">{formatCurrency(disclosure.administrative_expenses)} ر.س</p>
                        </div>
                        <div className="p-2 bg-muted/50 rounded">
                          <p className="text-muted-foreground text-xs">صيانة</p>
                          <p className="font-semibold">{formatCurrency(disclosure.maintenance_expenses)} ر.س</p>
                        </div>
                        <div className="p-2 bg-muted/50 rounded">
                          <p className="text-muted-foreground text-xs">تطوير</p>
                          <p className="font-semibold">{formatCurrency(disclosure.development_expenses)} ر.س</p>
                        </div>
                        <div className="p-2 bg-muted/50 rounded">
                          <p className="text-muted-foreground text-xs">أخرى</p>
                          <p className="font-semibold">{formatCurrency(disclosure.other_expenses)} ر.س</p>
                        </div>
                      </div>
                      {disclosure.vat_amount && disclosure.vat_amount > 0 && (
                        <div className="mt-2 p-2 bg-warning/10 rounded flex justify-between items-center">
                          ...
                          <span className="font-semibold text-warning">{formatCurrency(disclosure.vat_amount)} ر.س</span>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="py-0 pb-3">
                      <div className="grid grid-cols-3 gap-3 text-sm">
                        <div className="p-2 bg-chart-1/10 rounded text-center">
                          <p className="text-muted-foreground text-xs">حصة الناظر ({disclosure.nazer_percentage}%)</p>
                          <p className="font-semibold text-chart-1">{formatCurrency(disclosure.nazer_share)} ر.س</p>
                        </div>
                        <div className="p-2 bg-chart-2/10 rounded text-center">
                          <p className="text-muted-foreground text-xs">الثلث الخيري ({disclosure.charity_percentage}%)</p>
                          <p className="font-semibold text-chart-2">{formatCurrency(disclosure.charity_share)} ر.س</p>
                        </div>
                        <div className="p-2 bg-chart-3/10 rounded text-center">
                          <p className="text-muted-foreground text-xs">رقبة الوقف ({disclosure.corpus_percentage}%)</p>
                          <p className="font-semibold text-chart-3">{formatCurrency(disclosure.corpus_share)} ر.س</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="py-0 pb-3">
                      <div className="grid grid-cols-4 gap-3 text-sm text-center">
                        ...
                        <div className="p-2 bg-heir-daughter/10 rounded">
                          ...
                          <p className="font-semibold text-heir-daughter">{disclosure.daughters_count}</p>
                        </div>
                      ...
                      <CardContent className="py-0 pb-3">
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="p-2 bg-muted/50 rounded flex justify-between">
                            ...
                            <span className="font-semibold">{formatCurrency(disclosure.opening_balance)} ر.س</span>
                          </div>
                          <div className="p-2 bg-primary/10 rounded flex justify-between">
                            <span className="text-muted-foreground">الرصيد الختامي</span>
                            <span className="font-semibold text-primary">{formatCurrency(disclosure.closing_balance)} ر.س</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
