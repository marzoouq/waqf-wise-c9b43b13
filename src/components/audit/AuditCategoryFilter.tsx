...
import { DollarSign, Users, Building2, Shield, UserCog, Settings, X } from 'lucide-react';
      ...
      {CATEGORIES.map((cat) => {
        ...
        return (
          <Button
            ...
            variant="outline"
            size="sm"
            className={cn('gap-2 transition-all', isSelected && cat.color)}
            onClick={() => onCategoryChange(isSelected ? null : cat.id)}
          >
            <Icon className="h-4 w-4" />
            {cat.label}
            {isSelected && <X className="h-3 w-3 me-1" />}
          </Button>
        );
      })}
