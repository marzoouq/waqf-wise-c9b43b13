...
export function FiscalYearSummaryCard({ fiscalYearId, closing }: FiscalYearSummaryCardProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useFiscalYearSummary(fiscalYearId, !!closing);
  ...
  if (error) {
    return (
      <ErrorState title="خطأ في التحميل" message="فشل تحميل ملخص السنة المالية" onRetry={refetch} />
    );
  }
  ...
  const stats = [
    ...
    {
      label: 'صافي الدخل',
      value: closing?.net_income || (summary?.total_revenues || 0) - (summary?.total_expenses || 0),
      icon: DollarSign,
      color: 'text-info',
      bgColor: 'bg-info/10',
    },
    ...
    },
    {
      ...
      value: closing?.total_beneficiary_distributions || summary?.beneficiary_distributions || 0,
      icon: Users,
      color: 'text-warning',
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-2xl font-bold">
                    ...
                    <span className="text-sm font-normal text-muted-foreground me-1">ر.س</span>
                  </p>
                </div>
              </div>
            </CardContent>
