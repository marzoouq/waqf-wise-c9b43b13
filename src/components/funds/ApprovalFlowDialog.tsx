...
export function ApprovalFlowDialog({
  ...
  const { user } = useAuth();
  const { approvals, addApproval, getCurrentLevel } = useDistributionApprovals(distributionId);
  ...
  return (
    <ResponsiveDialog
      ...
      title="مسار موافقات التوزيع"
      description={`توزيع ${distributionMonth} - المبلغ: ${distributionAmount.toLocaleString()} ر.س`}
      size="lg"
    >
                        ...
                        <>
                          ...
                          {approval.approved_at && (
                            <p className="text-xs text-muted-foreground">
                              {format(new Date(approval.approved_at), 'dd MMMM yyyy - HH:mm', {
                                locale: ar,
                              })}
                            </p>
                          )}
                          {approval.notes && (
                            <p className="text-sm mt-2 p-2 bg-muted rounded-md">{approval.notes}</p>
                          )}
                        </>
                      )}
