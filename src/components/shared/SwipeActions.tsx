...
export function SwipeActions({
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (newTranslateX < 0) {
        ...
        newTranslateX = hasRightAction ? Math.max(newTranslateX, -maxSwipeLeft - 20) : 0;
      } else {
        // سحب لليمين (يظهر الإجراء الأيسر)
        newTranslateX = hasLeftAction ? Math.min(newTranslateX, maxSwipeRight + 20) : 0;
      }
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    const shouldTriggerAction =
      Math.abs(translateX) >= threshold || swipeState.velocity >= VELOCITY_THRESHOLD;
  ...
  return (
    <div ref={containerRef} className={cn('relative overflow-hidden touch-pan-y', className)}>
      {/* الإجراء الأيسر */}
      {hasLeftAction && (
        <div
          className={cn(
