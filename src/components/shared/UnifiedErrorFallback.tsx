...
export function UnifiedErrorFallback({
  ...
  const displayMessage =
    message || (componentName ? `${defaults.message} (${componentName})` : defaults.message);
    ...
    >
      <Card className={cn('shadow-lg', styles.borderColor)}>
        ...
        <CardContent className="space-y-4">
          <CardDescription className="text-base leading-relaxed">{displayMessage}</CardDescription>
          ...
          {showDetails && error && (
            <details className="p-4 bg-muted rounded-lg text-xs">
              ...
              <div className="space-y-2 overflow-auto">
                <p>
                  <strong>النوع:</strong> {error.name}
                </p>
                <p>
                  <strong>الرسالة:</strong> {error.message}
                </p>
                {error.stack && (
                  <pre className="whitespace-pre-wrap text-muted-foreground">{error.stack}</pre>
                )}
              </div>
            </details>
          )}
          ...
          <div className="flex gap-3 pt-2">
            {showRetry && onRetry && (
              <Button onClick={handleRetry} className="flex-1 gap-2" disabled={isRetrying}>
