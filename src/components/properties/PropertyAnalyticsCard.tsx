...
export function PropertyAnalyticsCard({ property, onAnalyzeClick }: PropertyAnalyticsCardProps) {
  ...
  const getPerformanceStatus = () => {
    if (occupancyRate >= 90)
      return { label: 'ممتاز', variant: 'default' as const, color: 'text-success' };
    if (occupancyRate >= 70)
      return { label: 'جيد', variant: 'secondary' as const, color: 'text-primary' };
    if (occupancyRate >= 50)
      return { label: 'متوسط', variant: 'outline' as const, color: 'text-warning' };
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <div className={`text-2xl font-bold ${performance.color}`}>{occupancyRate}%</div>
          </div>
          <div className="space-y-1">
        ...
        <Button className="w-full" variant="outline" onClick={onAnalyzeClick}>
