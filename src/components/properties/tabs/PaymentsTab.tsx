...
export const PaymentsTab = ({ onEdit }: Props) => {
  ...
  const getPaymentStatus = (payment: RentalPayment) => {
    ...
    if (dueDate < today && payment.status !== 'مدفوع' && !payment.payment_date) {
      return {
        status: 'متأخر',
        color: 'bg-destructive-light text-destructive border-destructive/30',
      };
    }
  ...
  const totalPaid =
    payments
      ?.filter((p) => p.status === 'مدفوع' || p.payment_date)
      .reduce((sum, p) => sum + Number(p.amount_paid), 0) || 0;
  const underCollectionPayments =
    payments
      ?.filter((p) => p.status === 'تحت التحصيل')
      .reduce((sum, p) => sum + Number(p.amount_due), 0) || 0;
  const totalDue =
    payments
      ?.filter((p) => p.status !== 'مدفوع' && !p.payment_date && p.status !== 'تحت التحصيل')
      .reduce((sum, p) => sum + Number(p.amount_due), 0) || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
        ...
        <Card className="p-3 sm:p-4">
          ...
          <div className="text-lg sm:text-2xl font-bold text-warning">
            {underCollectionPayments.toLocaleString('ar-SA')}{' '}
            <span className="text-xs sm:text-sm">ر.س</span>
          </div>
        </Card>
        ...
        <Card className="p-3 sm:p-4">
          ...
          <div className="text-lg sm:text-2xl font-bold text-destructive">
            {overduePayments.toLocaleString('ar-SA')}{' '}
            <span className="text-xs sm:text-sm">ر.س</span>
          </div>
        </Card>
      </div>
      ...
      <UnifiedDataTable
        ...
        columns={[
          ...
          },
          {
            ...
            label: 'الحالة',
            render: (_: unknown, row: RentalPayment) => (
              <Badge className={getPaymentStatus(row).color}>{getPaymentStatus(row).status}</Badge>
            ),
          },
        ]}
        data={paginatedData}
      ...
      <DeleteConfirmDialog
        ...
        itemName={
          paymentToDelete
            ? `${paymentToDelete.payment_number} - ${paymentToDelete.amount_due} ر.س`
            : ''
        }
