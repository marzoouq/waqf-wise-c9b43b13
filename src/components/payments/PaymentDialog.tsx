...
export function PaymentDialog({ open, onOpenChange, payment, onSave }: PaymentDialogProps) {
  ...
  const form = useForm<PaymentFormValues>({
    ...
    defaultValues: {
      ...
      payment_method:
        (payment?.payment_method as 'cash' | 'bank_transfer' | 'cheque' | 'card') || 'cash',
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input
                    placeholder={paymentType === 'receipt' ? 'اسم الدافع' : 'اسم المستفيد'}
                    {...field}
          ...
          <DialogFooter>
            ...
            <Button type="submit">{payment ? 'حفظ التعديلات' : 'إضافة السند'}</Button>
          </DialogFooter>
        </form>
      </Form>
    </ResponsiveDialog>
