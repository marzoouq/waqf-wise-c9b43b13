...
export function PerformanceKPICards({ stats, isLoading, alertsCount }: PerformanceKPICardsProps) {
  const cacheHitVariant = !stats
    ? 'default'
    : stats.cacheHitRatio >= 99
      ? 'success'
      : stats.cacheHitRatio >= 95
        ? 'warning'
        : 'destructive';
  ...
  const deadRowsVariant = !stats
    ? 'default'
    : stats.totalDeadRows > 10000
      ? 'destructive'
      : stats.totalDeadRows > 1000
        ? 'warning'
        : 'success';
  ...
  return (
    <UnifiedStatsGrid columns={3}>
      <UnifiedKPICard
        ...
        loading={isLoading}
        subtitle={
          stats?.cacheHitRatio >= 99 ? 'ممتاز' : stats?.cacheHitRatio >= 95 ? 'جيد' : 'يحتاج تحسين'
        }
      />
