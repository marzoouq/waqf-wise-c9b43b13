...
export const CreateRequestDialog = memo(
  ({ open, onOpenChange, onSuccess }: CreateRequestDialogProps) => {
    ...
    const { data: beneficiaries = [], isLoading: beneficiariesLoading } = useQuery<Beneficiary[]>({
      ...
      queryFn: async () => {
        ...
        if (beneficiarySearch.trim()) {
          query = query.or(
            `full_name.ilike.%${beneficiarySearch}%,national_id.ilike.%${beneficiarySearch}%`
          );
        }
          ...
          <div className="space-y-2">
            ...
            <Select value={beneficiaryId} onValueChange={setBeneficiaryId} name="beneficiary">
              <SelectTrigger id="beneficiary-select" aria-label="اختر المستفيد">
                <SelectValue
                  placeholder={beneficiariesLoading ? 'جاري التحميل...' : 'اختر المستفيد'}
                />
              </SelectTrigger>
              <SelectContent>
                {beneficiaries.map((b) => (
