...
import { usePendingApprovals } from '@/hooks/requests/usePendingApprovals';
  ...
  if (isError) {
    return (
      <Alert variant="destructive" className="mb-6">
        ...
        <AlertDescription>
          حدث خطأ في جلب الموافقات المعلقة:{' '}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1 sm:gap-2 mb-1">
                    ...
                    <Badge
                      className={`${getPriorityColor(approval.priority)} text-[10px] sm:text-xs`}
                      variant="secondary"
                    >
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 mt-1 text-[10px] sm:text-xs text-muted-foreground">
                    <span className="hidden sm:inline">
                      {format(approval.date, 'dd MMM yyyy', { locale: ar })}
                    </span>
                    <span className="sm:hidden">
                      {format(approval.date, 'dd/MM', { locale: ar })}
                    </span>
                    {approval.amount && (
