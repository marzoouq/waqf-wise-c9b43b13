...
export function FiscalYearNotPublishedBanner() {
  ...
  return (
    <Alert className="border-warning/30 bg-warning/10">
      ...
      <AlertTitle className="text-warning">السنة المالية الحالية غير منشورة بعد</AlertTitle>
      <AlertDescription className="text-warning/80">
        <div className="flex items-center gap-2 mt-1">
          <Calendar className="h-3 w-3" />
          <span>
            {activeFiscalYear.name} (
            {format(new Date(activeFiscalYear.start_date), 'yyyy/MM/dd', { locale: ar })} -{' '}
            {format(new Date(activeFiscalYear.end_date), 'yyyy/MM/dd', { locale: ar })})
          </span>
        </div>
        {lastPublishedYear && (
        ...
        <p className="mt-1 text-sm">
          سيتم نشر تفاصيل العقود والإيجارات والمصروفات بعد اعتماد الناظر. التوزيعات تظهر فور
          اعتمادها.
        </p>
      </AlertDescription>
    </Alert>
