...
import { FileText, FileSpreadsheet, Building2, Receipt, Download, User, Info } from 'lucide-react';
...
export function ReportsMenu({ type = 'beneficiary' }: ReportsMenuProps) {
  ...
  const exportPaymentsPDF = async () => {
    try {
      ...
      yPos += 7;
      doc.text(`رقم الهوية: ${beneficiary?.national_id || ''}`, pageWidth - 20, yPos, {
        align: 'right',
      });
      yPos += 7;
      ...
      yPos += 7;
      doc.text(`التاريخ: ${new Date().toLocaleDateString('ar-SA')}`, pageWidth - 20, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      if (payments && payments.length > 0) {
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(
          `إجمالي المدفوعات: ${totalAmount.toLocaleString('ar-SA')} ر.س`,
          pageWidth - 20,
          yPos,
          { align: 'right' }
        );
        ...
        yPos += 7;
        doc.text(`متوسط الدفعة: ${avgAmount.toLocaleString('ar-SA')} ر.س`, pageWidth - 20, yPos, {
          align: 'right',
        });
        yPos += 10;
        ...
        doc.text('سيتم تحديث هذا التقرير عند إضافة مدفوعات جديدة', pageWidth / 2, yPos + 20, {
          align: 'center',
        });
        addWaqfFooter(doc, fontName);
      }
  ...
  const exportAccountStatement = async () => {
    try {
      ...
      doc.text(`الجوال: ${beneficiary?.phone || ''}`, pageWidth - 20, yPos, { align: 'right' });
      yPos += 7;
      doc.text(`البريد: ${beneficiary?.email || 'لا يوجد'}`, pageWidth - 20, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`الفئة: ${beneficiary?.category || ''}`, pageWidth - 20, yPos, { align: 'right' });
      ...
      doc.text(`الحالة: ${beneficiary?.status || ''}`, pageWidth - 20, yPos, { align: 'right' });
      yPos += 7;
      doc.text(`البنك: ${beneficiary?.bank_name || 'لا يوجد'}`, pageWidth - 20, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`الآيبان: ${beneficiary?.iban || 'لا يوجد'}`, pageWidth - 20, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      if (payments && payments.length > 0) {
        ...
        doc.setTextColor(...WAQF_COLORS.primary);
        doc.text(
          `الإجمالي الكلي: ${totalAmount.toLocaleString('ar-SA')} ر.س`,
          pageWidth - 20,
          finalY + 10,
          { align: 'right' }
        );
        ...
        doc.setFontSize(12);
        doc.setTextColor(...WAQF_COLORS.muted);
        doc.text('لا توجد معاملات مالية مسجلة حتى الآن', pageWidth / 2, yPos + 10, {
          align: 'center',
        });
        doc.text('سيتم تحديث الكشف عند إضافة معاملات جديدة', pageWidth / 2, yPos + 20, {
          align: 'center',
        });
        addWaqfFooter(doc, fontName);
      }
  ...
  const exportPersonalDataPDF = async () => {
    try {
      ...
      yPos += 7;
      doc.text(`رقم الهوية الوطنية: ${beneficiary.national_id}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`الجنس: ${beneficiary.gender || 'غير محدد'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(
        `تاريخ الميلاد: ${beneficiary.date_of_birth ? new Date(beneficiary.date_of_birth).toLocaleDateString('ar-SA') : 'غير محدد'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(`الجنسية: ${beneficiary.nationality || 'غير محددة'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      yPos += 7;
      doc.text(`البريد الإلكتروني: ${beneficiary.email || 'لا يوجد'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`المدينة: ${beneficiary.city || 'غير محددة'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`العنوان: ${beneficiary.address || 'غير محدد'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(
        `الحالة الاجتماعية: ${beneficiary.marital_status || 'غير محددة'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(`عدد الأبناء: ${beneficiary.number_of_sons || 0}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`عدد البنات: ${beneficiary.number_of_daughters || 0}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`عدد الزوجات: ${beneficiary.number_of_wives || 0}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(`حجم الأسرة: ${beneficiary.family_size || 0}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(
        `الحالة الوظيفية: ${beneficiary.employment_status || 'غير محددة'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(
        `الدخل الشهري: ${beneficiary.monthly_income ? beneficiary.monthly_income.toLocaleString('ar-SA') + ' ر.س' : 'غير محدد'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(`نوع السكن: ${beneficiary.housing_type || 'غير محدد'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 10;
      ...
      yPos += 7;
      doc.text(`القبيلة: ${beneficiary.tribe || 'غير محددة'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      ...
      yPos += 7;
      doc.text(
        `مستوى الأولوية: ${beneficiary.priority_level || 'غير محدد'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(
        `تاريخ القيد: ${new Date(beneficiary.created_at).toLocaleDateString('ar-SA')}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      ...
      doc.setFontSize(11);
      doc.setTextColor(0, 0, 0);
      doc.text(`البنك: ${beneficiary.bank_name || 'لم يتم التحديث'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
      yPos += 7;
      doc.text(
        `رقم الحساب: ${beneficiary.bank_account_number || 'لم يتم التحديث'}`,
        pageWidth - 25,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(`الآيبان: ${beneficiary.iban || 'لم يتم التحديث'}`, pageWidth - 25, yPos, {
        align: 'right',
      });
  ...
  const exportPropertiesReportPDF = async () => {
    try {
      ...
      yPos += 7;
      doc.text(
        `الإيجار الشهري: ${totalMonthlyRent.toLocaleString('ar-SA')} ر.س`,
        pageWidth - 20,
        yPos,
        { align: 'right' }
      );
      yPos += 7;
      doc.text(
        `الإيجار السنوي: ${totalAnnualRent.toLocaleString('ar-SA')} ر.س`,
        pageWidth - 20,
        yPos,
        { align: 'right' }
      );
  ...
  const beneficiaryReports = (
    <div className="space-y-4">
      <Alert>
        ...
        <AlertDescription>
          التقارير الشخصية تعمل على البيانات الموجودة فعلياً. بعض التقارير قد تكون فارغة إذا لم يتم
          تسجيل مدفوعات بعد.
        </AlertDescription>
      </Alert>
      ...
      <div className="space-y-2">
        ...
        <Button onClick={exportPersonalDataPDF} className="w-full justify-start" variant="outline">
          <User className="ms-2 h-4 w-4" />
          تقرير البيانات الشخصية (PDF)
        </Button>
        <Button onClick={exportPaymentsPDF} className="w-full justify-start" variant="outline">
          <FileText className="ms-2 h-4 w-4" />
          تقرير المدفوعات (PDF)
        </Button>
        <Button onClick={exportAccountStatement} className="w-full justify-start" variant="outline">
      ...
      <div className="space-y-2">
        ...
        <Button onClick={exportPropertiesExcel} className="w-full justify-start" variant="outline">
