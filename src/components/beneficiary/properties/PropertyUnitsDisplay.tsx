...
export function PropertyUnitsDisplay({ propertyId }: PropertyUnitsDisplayProps) {
  ...
  if (!units || units.length === 0) {
    return (
      <div className="text-center py-6 text-muted-foreground">لا توجد وحدات مسجلة لهذا العقار</div>
    );
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 mt-2">
      {units.map((unit) => {
        ...
        return (
          <Card
            key={unit.id}
            className={`border-s-4 ${isOccupied ? 'border-s-success' : 'border-s-border'}`}
          >
