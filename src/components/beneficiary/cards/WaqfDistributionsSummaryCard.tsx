...
export function WaqfDistributionsSummaryCard({ beneficiaryId }: WaqfDistributionsSummaryCardProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useWaqfDistributionsSummary(beneficiaryId);
  ...
  const currentYearAmount = currentYearDistributions.reduce(
    (sum, d) => sum + (d.share_amount || 0),
    0
  );
  const historicalAmount = historicalDistributions.reduce(
    (sum, d) => sum + (d.share_amount || 0),
    0
  );
  ...
  const distributionsByYear = distributions.reduce(
    (acc, dist) => {
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <Card className="bg-gradient-to-br from-primary/5 via-background to-primary/10 border-primary/20 h-full cursor-pointer hover:shadow-md transition-shadow">
          ...
          <CardContent className="p-3 sm:p-4 pt-0">
            ...
            <div className="text-2xl sm:text-3xl font-bold text-primary tabular-nums mb-3">
              ...
              <span className="text-sm sm:text-base font-normal text-muted-foreground me-1">
                {' '}
                ر.س
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {currentYearAmount > 0 && (
                <Badge
                  variant="outline"
                  className="bg-[hsl(var(--status-success)/0.1)] text-[hsl(var(--status-success))] border-[hsl(var(--status-success)/0.3)]"
                >
      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        ...
        <ScrollArea className="max-h-[60vh]">
          {yearsData.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">لا توجد توزيعات مسجلة</div>
          ) : (
            <div className="space-y-4">
              {yearsData.map((year) => (
                ...
                >
                  <CardHeader
                    className={`py-3 ${
                      year.isClosed ? 'bg-muted/50' : 'bg-[hsl(var(--status-success)/0.1)]'
                    }`}
                  >
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant={year.isClosed ? 'secondary' : 'default'}
                          className="text-xs"
                        >
                      ...
                      <span
                        className={`font-bold ${year.isClosed ? 'text-muted-foreground' : 'text-[hsl(var(--status-success))]'}`}
                      >
