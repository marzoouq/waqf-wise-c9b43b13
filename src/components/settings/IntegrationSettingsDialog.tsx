...
export function IntegrationSettingsDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from(tableName).update(updateData).eq('id', integration.id);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>تكوين إعدادات التكامل والاتصال</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="flex items-center justify-between p-3 border rounded-lg">
            <div>
              ...
              <p className="text-sm text-muted-foreground">تفعيل أو تعطيل هذا التكامل</p>
            </div>
