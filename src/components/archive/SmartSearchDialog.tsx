...
import { useSmartSearch } from '@/hooks/archive/useDocumentTags';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh]">
        ...
        <Tabs
          value={searchType}
          onValueChange={(v) => setSearchType(v as 'text' | 'tags' | 'ocr')}
          className="w-full"
        >
          ...
          <TabsContent value={searchType} className="mt-4 space-y-4">
            ...
            {smartSearch.data && smartSearch.data.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">لم يتم العثور على نتائج</div>
            )}
