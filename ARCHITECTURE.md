# 🏗️ هيكل مشروع منصة إدارة الوقف

> **الإصدار**: 2.9.24 | **آخر تحديث**: 2025-12-16

📖 **للمطورين والـ AI Agents**: راجع [`AI_CODING_AGENT.md`](AI_CODING_AGENT.md) للدليل الشامل

## 📁 هيكل المجلدات

```
src/
├── assets/              # الصور والأيقونات
├── components/          # المكونات القابلة لإعادة الاستخدام
│   ├── accounting/      # مكونات المحاسبة
│   ├── archive/         # مكونات الأرشفة
│   ├── auth/            # مكونات المصادقة
│   ├── beneficiary/     # مكونات المستفيدين
│   ├── contracts/       # مكونات العقود
│   ├── dashboard/       # مكونات لوحات التحكم
│   ├── distributions/   # مكونات التوزيعات
│   ├── families/        # مكونات العائلات
│   ├── invoices/        # مكونات الفواتير
│   ├── loans/           # مكونات القروض
│   ├── permissions/     # مكونات الصلاحيات
│   ├── properties/      # مكونات العقارات
│   ├── requests/        # مكونات الطلبات
│   ├── settings/        # مكونات الإعدادات
│   └── ui/              # مكونات Shadcn UI
├── config/              # إعدادات التطبيق
├── contexts/            # React Contexts
├── hooks/               # Custom Hooks
│   ├── accounting/      # hooks المحاسبة
│   ├── auth/            # hooks المصادقة
│   ├── beneficiary/     # hooks المستفيدين
│   └── ...
├── integrations/        # تكاملات خارجية (Supabase)
├── lib/                 # المكتبات والأدوات
│   ├── cache/           # إدارة الذاكرة المؤقتة
│   ├── errors/          # معالجة الأخطاء
│   ├── logger/          # نظام السجلات
│   ├── monitoring/      # مراقبة النظام
│   ├── optimization/    # تحسين الأداء
│   ├── pdf/             # إنشاء PDF
│   └── utils/           # أدوات مساعدة
├── pages/               # صفحات التطبيق
├── routes/              # توجيهات التطبيق
└── types/               # أنواع TypeScript
```

## 🔐 نظام الأدوار والصلاحيات

### الأدوار المتاحة
| الدور | الوصف | المسؤوليات |
|-------|-------|------------|
| `nazer` | الناظر | صلاحيات كاملة، الموافقات النهائية |
| `admin` | المشرف | إدارة المستخدمين والإعدادات |
| `accountant` | المحاسب | القيود المالية والتقارير |
| `cashier` | أمين الصندوق | معالجة المدفوعات |
| `archivist` | الأرشيفي | إدارة المستندات |
| `beneficiary` | المستفيد | عرض البيانات الشخصية |
| `waqf_heir` | وارث الوقف | شفافية كاملة للبيانات |

### دوال التحقق من الصلاحيات
```typescript
// التحقق من دور
public.is_admin_or_nazer()
public.is_staff_only()
public.is_financial_staff()
public.is_accountant()
public.is_cashier()
public.is_archivist()
public.has_role(user_id, role)
```

## 📊 الجداول الرئيسية

### جداول المستخدمين
- `profiles` - ملفات المستخدمين
- `user_roles` - أدوار المستخدمين
- `user_sessions` - جلسات المستخدمين
- `two_factor_auth` - المصادقة الثنائية

### جداول المستفيدين
- `beneficiaries` - بيانات المستفيدين
- `families` - العائلات
- `heir_distributions` - توزيعات الورثة

### جداول المحاسبة
- `accounts` - شجرة الحسابات
- `journal_entries` - القيود اليومية
- `journal_entry_lines` - بنود القيود
- `fiscal_years` - السنوات المالية
- `bank_accounts` - الحسابات البنكية

### جداول العقارات
- `properties` - العقارات
- `property_units` - الوحدات العقارية
- `contracts` - العقود
- `rental_payments` - دفعات الإيجار

### جداول التوزيعات
- `distributions` - التوزيعات
- `distribution_details` - تفاصيل التوزيع
- `distribution_approvals` - موافقات التوزيع

## 🔄 مسارات الموافقات

### مسار التوزيع
```
إنشاء التوزيع → محاكاة → مراجعة المحاسب → موافقة الناظر → تنفيذ
```

### مسار القروض
```
تقديم الطلب → مراجعة → موافقة المحاسب → موافقة الناظر → الصرف
```

### مسار الفزعات
```
تقديم الطلب → تقييم → موافقة → صرف فوري
```

## 🛡️ سياسات الأمان (RLS)

### مبادئ الأمان
1. **العزل الكامل**: المستفيد يرى بياناته فقط
2. **الشفافية للورثة**: ورثة الوقف يرون جميع البيانات
3. **صلاحيات الموظفين**: حسب الدور والوظيفة
4. **حماية البيانات الحساسة**: تشفير وتحكم صارم

### أمثلة على السياسات
```sql
-- المستفيد يرى بياناته فقط
CREATE POLICY "beneficiary_own_data" ON beneficiaries
FOR SELECT USING (user_id = auth.uid());

-- الموظفين يرون جميع البيانات
CREATE POLICY "staff_all_data" ON beneficiaries
FOR SELECT USING (public.is_staff_only());
```

## ⚡ تحسينات الأداء

### استراتيجيات التحسين
1. **React Query**: تخزين مؤقت ذكي (staleTime: 2min)
2. **Lazy Loading**: تحميل كسول للمكونات
3. **Promise.all**: استعلامات متوازية
4. **Debouncing**: تأخير البحث
5. **Virtualization**: قوائم افتراضية للبيانات الكبيرة

### ملفات التحسين
- `src/lib/queryOptimization.ts` - تحسين الاستعلامات
- `src/lib/lazyWithRetry.ts` - التحميل الكسول
- `src/lib/imageOptimization.ts` - تحسين الصور

## 📱 التصميم المتجاوب

### نقاط التوقف
- `sm`: 640px (موبايل)
- `md`: 768px (تابلت)
- `lg`: 1024px (لابتوب)
- `xl`: 1280px (شاشة كبيرة)

### مكونات الموبايل
- `MobileCard` - بطاقات للموبايل
- `MobileOptimizedLayout` - تخطيط محسن
- `BottomNavigation` - تنقل سفلي

## 🔧 أدوات التطوير

### Scripts
```bash
npm run dev              # تشغيل التطوير
npm run build            # بناء الإنتاج
npm run lint             # فحص الكود
npx vitest run           # تشغيل الاختبارات
npx vitest run --coverage # الاختبارات مع التغطية
npx playwright test      # اختبارات E2E
```

### الأدوات المستخدمة
- **React Query DevTools**: مراقبة الاستعلامات
- **TypeScript**: نظام الأنواع
- **ESLint + Prettier**: تنسيق الكود
- **Vitest**: اختبارات الوحدة (408+ اختبار)
- **Playwright**: اختبارات E2E

## 📝 معايير الكود

### تسمية الملفات
- المكونات: `PascalCase.tsx`
- الـ Hooks: `useCamelCase.ts`
- الأدوات: `camelCase.ts`
- الأنواع: `camelCase.ts`

### هيكل المكون
```typescript
// 1. الاستيرادات
import { ... } from '...';

// 2. الأنواع
interface Props { ... }

// 3. المكون
export function Component({ ... }: Props) {
  // 3.1 الـ Hooks
  // 3.2 المتغيرات المحلية
  // 3.3 الدوال
  // 3.4 العرض
  return <div>...</div>;
}
```

## 🗓️ السنوات المالية

### السنة المالية 2024-2025
- **الفترة**: 25/10/2024 - 24/10/2025
- **الحالة**: مغلقة (للقراءة فقط)
- **إجمالي التوزيعات**: 995,000 ر.س

### السنة المالية 2025-2026
- **الفترة**: 25/10/2025 - مستمرة
- **الحالة**: نشطة

## 📞 الدعم والمساعدة

للمساعدة التقنية:
- مراجعة [`AI_CODING_AGENT.md`](AI_CODING_AGENT.md) - دليل شامل للـ AI Agents والمطورين
- مراجعة [`docs/ARCHITECTURE_RULES.md`](docs/ARCHITECTURE_RULES.md) - قواعد صارمة
- مراجعة `src/hooks/README.md` للـ Hooks
- مراجعة `src/services/README.md` للـ Services
- مراجعة `src/lib/query-keys.ts` للـ Query Keys (370+)
