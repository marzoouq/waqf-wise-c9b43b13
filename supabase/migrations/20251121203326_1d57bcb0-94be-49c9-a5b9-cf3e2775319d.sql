...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM families WHERE family_name = 'عائلة الثبيتي') THEN
    ...
    VALUES (
      'عائلة الثبيتي',
      (SELECT id FROM beneficiaries WHERE gender = 'ذكر' AND relationship = 'ابن' ORDER BY created_at ASC LIMIT 1),
      'قبيلة الثبيتي',
      'نشط',
      14,
    ...
    GET DIAGNOSTICS family_members_created = ROW_COUNT;
  END IF;
  ...
  result := json_build_object(
    ...
    'family_members_created', family_members_created,
