-- المرحلة 1: Triggers للتوزيعات والمدفوعات (بدون RLS)

-- 1. Function لتحديث حالة التوزيع تلقائياً عند اكتمال الموافقات
...
DECLARE
  ...
  approved_count INT;
BEGIN
  -- حساب عدد الموافقات المطلوبة والمعتمدة
  ...
  -- إذا تمت جميع الموافقات، تحديث حالة التوزيع
...
DROP TRIGGER IF EXISTS trigger_auto_update_distribution_status ON distribution_approvals;
CREATE TRIGGER trigger_auto_update_distribution_status
AFTER UPDATE ON distribution_approvals
FOR EACH ROW
WHEN (NEW.status = 'موافق' AND OLD.status != 'موافق')
EXECUTE FUNCTION auto_update_distribution_status();

-- 2. Function لإنشاء قيد محاسبي تلقائياً عند اعتماد التوزيع
...
BEGIN
  IF NEW.status = 'معتمد' AND (OLD.status IS NULL OR OLD.status != 'معتمد') AND NEW.journal_entry_id IS NULL THEN
    ...
    INSERT INTO journal_entries (
      ...
      'توزيع معتمد - ' || NEW.month || ' - عدد المستفيدين: ' || NEW.beneficiaries_count,
      'distribution',
      'posted',
      'distribution',
      NEW.id,
    ...
    INSERT INTO activities (user_name, action, timestamp)
    VALUES ('النظام', 'تم إنشاء قيد محاسبي تلقائي للتوزيع - ' || NEW.month, NOW());
  END IF;
...
DROP TRIGGER IF EXISTS trigger_create_journal_for_distribution ON distributions;
CREATE TRIGGER trigger_create_journal_for_distribution
BEFORE UPDATE ON distributions
FOR EACH ROW
EXECUTE FUNCTION create_journal_entry_for_distribution();

-- 3. Function لإنشاء قيد محاسبي تلقائياً للمدفوعات
...
BEGIN
  IF NEW.journal_entry_id IS NULL AND NEW.amount > 0 THEN
    ...
    INSERT INTO journal_entries (
      ...
      entry_number,
      NEW.payment_date,
      CASE WHEN NEW.payment_type = 'receipt' THEN 'سند قبض - ' ELSE 'سند صرف - ' END 
      || NEW.description || ' - ' || NEW.payer_name,
      'payment',
      'posted',
      'payment',
      NEW.id,
...
CREATE TRIGGER trigger_create_journal_for_payment
BEFORE INSERT ON payments
FOR EACH ROW
EXECUTE FUNCTION create_journal_entry_for_payment();

-- 4. Function لإرسال إشعارات تلقائية
...
BEGIN
  IF NEW.status = 'معتمد' AND (OLD.status IS NULL OR OLD.status != 'معتمد') THEN
    INSERT INTO notifications (
      ...
      'تم اعتماد التوزيع',
      'تم اعتماد توزيع شهر ' || NEW.month || ' بمبلغ ' || NEW.total_amount || ' ريال',
      'distribution_approved',
      'high',
      ARRAY['nazer', 'accountant', 'admin'],
      'distribution',
      NEW.id
    );
  END IF;
...
DROP TRIGGER IF EXISTS trigger_send_distribution_notification ON distributions;
CREATE TRIGGER trigger_send_distribution_notification
AFTER UPDATE ON distributions
FOR EACH ROW
EXECUTE FUNCTION send_distribution_notification();