  ...
  try {
    ...
    if (otpError || !otpRecord) {
      ...
      if (directError || !directContract) {
        return new Response(JSON.stringify({ error: 'رقم العقد غير صحيح' }), {
          status: 401,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (!tenantPhone.includes(cleanPhone) && !cleanPhone.includes(tenantPhone.slice(-9))) {
        return new Response(JSON.stringify({ error: 'رقم الهاتف لا يتطابق مع العقد' }), {
          status: 401,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      // إنشاء جلسة جديدة
      const { error: sessionError } = await supabaseAdmin.from('tenant_sessions').insert({
    ...
    // حذف الجلسات القديمة
    await supabaseAdmin.from('tenant_sessions').delete().eq('tenant_id', otpRecord.tenant_id);
