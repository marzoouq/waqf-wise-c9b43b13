...
function getCorsHeadersForOrigin(origin: string | null): Record<string, string> {
  ...
  const allowedOrigin = isAllowedOrigin(origin) ? origin : ALLOWED_ORIGINS[0]; // الـ origin الافتراضي

  return {
    'Access-Control-Allow-Origin': allowedOrigin || '*',
    'Access-Control-Allow-Headers':
      'authorization, x-client-info, apikey, content-type, x-cron-job',
 ...
 */
export function successResponse<T>(data: T, message?: string, req?: Request): Response {
  return jsonResponse(
    {
      success: true,
      message,
...
export function rateLimitResponse(
  message: string = 'تجاوزت الحد المسموح',
  req?: Request
): Response {
