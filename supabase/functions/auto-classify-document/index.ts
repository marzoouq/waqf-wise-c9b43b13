...
const classificationCategories = [
  { id: 'contract', name: 'عقود', keywords: ['عقد', 'اتفاقية', 'إيجار', 'تأجير', 'شراء', 'بيع'] },
  {
    id: 'financial',
    name: 'مالية',
    keywords: ['فاتورة', 'سند', 'قبض', 'صرف', 'إيراد', 'مصروف', 'حساب'],
  },
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[auto-classify-document] ✅ Authorized:', {
      userId: user.id,
    ...
    return errorResponse(error instanceof Error ? error.message : 'خطأ في التصنيف التلقائي', 500);
  }
});
