...
// ============ Input Validation ============
function validateDaysBeforeExpiry(value: unknown): {
  valid: boolean;
  value: number;
  error?: string;
} {
...
serve(async (req) => {
  ...
  try {
    ...
    if (cronSecret && expectedCronSecret && cronSecret === expectedCronSecret) {
      ...
      if (!rateLimitResult.allowed) {
        ...
        return errorResponse(
          `تجاوز الحد المسموح للمهام المجدولة. يرجى الانتظار ${Math.ceil(rateLimitResult.resetIn / 60000)} دقيقة.`,
          429
        );
      }
    }
    ...
    if (!isAuthorized) {
      ...
      if (!hasAccess) {
        console.warn(
          `[contract-renewal-alerts] ❌ Forbidden - User ${user.id} lacks required role (has: ${roles?.map((r) => r.role).join(', ') || 'none'})`
        );
      ...
      console.log(
        `[contract-renewal-alerts] ✅ Authorized via JWT - User: ${user.id}, Roles: ${roles?.map((r) => r.role).join(', ')}`
      );
      ...
      if (!rateLimitResult.allowed) {
        ...
        return errorResponse(
          `تجاوزت الحد المسموح (${RATE_LIMIT_USER} طلبات/ساعة). يرجى الانتظار ${Math.ceil(rateLimitResult.resetIn / 60000)} دقيقة.`,
          429
        );
      }
    }
    ...
    console.log(
      `[contract-renewal-alerts] Checking contracts expiring in ${daysBeforeExpiry} days... (auth: ${authMethod})`
    );
    ...
    console.log(
      `[contract-renewal-alerts] Found ${expiringContracts?.length || 0} contracts expiring soon`
    );
    ...
    for (const contract of expiringContracts || []) {
      ...
      if (!existingAlert) {
        ...
        const props = contract.properties as
...
الإيجار الشهري: ${contract.monthly_rent?.toLocaleString('ar-SA')} ريال
        ...
        console.log(
          `[contract-renewal-alerts] Alert created for contract ${contract.contract_number}, notified ${admins?.length || 0} admins`
        );
      }
    }
    ...
    return errorResponse(error instanceof Error ? error.message : 'خطأ في فحص العقود', 500);
  }
});
