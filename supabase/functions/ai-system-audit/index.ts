...
serve(async (req) => {
  ...
  try {
    ...
    const autoFixResults: Record<string, unknown>[] = [];
...
async function gatherSystemData(supabase: ReturnType<typeof createClient>, categories: string[]) {
    ...
    try {
      ...
      if (Array.isArray(directParse)) {
        ...
        return directParse.map((f: Record<string, unknown>) => ({
          ...f,
          categoryLabel: CATEGORY_LABELS[String(f.category)] || f.category,
          fixed: false
        }));
      }
    } catch (e) {
...
function buildAnalysisPrompt(systemData: Record<string, unknown>, categories: string[]): string {
  ...
  const dataToSend = {
    ...
    tablesWithoutRLS: systemData.tables?.filter((t: Record<string, unknown>) => !t.has_rls)?.map((t: Record<string, unknown>) => t.table_name) || []
  };
...
function performEnhancedLocalAnalysis(systemData: Record<string, unknown>, categories: string[]): AuditFinding[] {
    ...
    if (systemData.tables && Array.isArray(systemData.tables) && systemData.tables.length > 0) {
      ...
      if (systemData.indexes && Array.isArray(systemData.indexes)) {
        largeTables.forEach((table: Record<string, unknown>) => {
          const tableIndexes = systemData.indexes.filter((i: Record<string, unknown>) => i.table_name === table.table_name);
  ...
  if (systemData.errorLogs && Array.isArray(systemData.errorLogs) && systemData.errorLogs.length > 0) {
    const errorCounts = systemData.errorLogs.reduce((acc: Record<string, number>, log: Record<string, unknown>) => {
      const message = log.message as string | undefined;
      const key = String(log.error_type || message?.slice(0, 50) || 'unknown');
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
  ...
  if (categories.includes('roles') && systemData.roles && Array.isArray(systemData.roles)) {
    const usersWithoutRoles = systemData.roles?.filter((r: Record<string, unknown>) => !r.role) || [];
...
async function savePendingFixes(supabase: ReturnType<typeof createClient>, auditId: string, findings: AuditFinding[]) {
...
async function sendSlackNotification(supabase: ReturnType<typeof createClient>, auditId: string, summary: Record<string, number>, criticalFindings: AuditFinding[]) {
  try {
