...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isAuthorized) {
      ...
      if (!rateLimitResult.allowed) {
        ...
        return errorResponse(
          `تجاوزت الحد المسموح (مرة واحدة/أسبوع). يرجى الانتظار ${daysRemaining} يوم.`,
          429
        );
      }
    }
    ...
    const { data: fullCleanupResult, error: fullCleanupError } =
      await supabase.rpc('run_full_cleanup');
    ...
    const { data: vacuumResult, error: vacuumError } = await supabase.rpc('run_vacuum_analyze');
    ...
    const { data: performanceHealth, error: healthCheckError } =
      await supabase.rpc('get_performance_health');
    ...
    const { data: unusedIndexes, error: unusedIndexesError } = await supabase.rpc(
      'get_unused_indexes_count'
    );
    ...
    const { data: highDeadRows, error: deadRowsError } = await supabase.rpc(
    ...
    const { data: idleConnections, error: idleError } = await supabase.rpc('get_idle_connections');
