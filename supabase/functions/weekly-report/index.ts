...
async function generateWeeklyReport(supabase: any): Promise<WeeklyStats> {
  ...
  const totalRevenue = revenues.reduce(
    (sum: number, r: { amount_due?: number }) => sum + (r.amount_due || 0),
    0
  );
  const totalExpenses = expenses.reduce(
    (sum: number, e: { credit_amount?: number }) => sum + (e.credit_amount || 0),
    0
  );
  const distributionAmount = distributions.reduce(
    (sum: number, d: { allocated_amount?: number }) => sum + (d.allocated_amount || 0),
    0
  );
      ...
      if (user && !checkRateLimit(user.id)) {
        ...
        return new Response(
          JSON.stringify({
            ...
            message: 'تجاوزت الحد المسموح (5 تشغيلات/ساعة)',
          }),
