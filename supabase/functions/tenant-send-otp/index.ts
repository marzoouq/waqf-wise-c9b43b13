...
serve(async (req) => {
  ...
  try {
    ...
    if (tenantError) {
      ...
      return new Response(JSON.stringify({ error: 'حدث خطأ أثناء البحث' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!tenant) {
      return new Response(JSON.stringify({ error: 'رقم الهاتف غير مسجل في النظام' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (contractError) {
      ...
      return new Response(JSON.stringify({ error: 'حدث خطأ أثناء البحث عن العقود' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!contracts || contracts.length === 0) {
      return new Response(JSON.stringify({ error: 'لا يوجد عقد نشط مرتبط بهذا الرقم' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await supabaseAdmin.from('tenant_otp_codes').delete().eq('tenant_id', tenant.id);

    // إنشاء سجل تحقق جديد (نستخدم رقم العقد كرمز)
    const { error: otpError } = await supabaseAdmin.from('tenant_otp_codes').insert({
      tenant_id: tenant.id,
      phone: cleanPhone,
    ...
    if (otpError) {
      ...
      return new Response(JSON.stringify({ error: 'فشل في إنشاء سجل التحقق' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
