...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-tenant-session',
};
  ...
  try {
    ...
    if (action === 'create-request' && req.method === 'POST') {
      ...
      if (createError) {
        ...
        return new Response(JSON.stringify({ error: 'فشل في إنشاء الطلب' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (action === 'rate-request' && req.method === 'POST') {
      ...
      if (updateError) {
        return new Response(JSON.stringify({ error: 'فشل في حفظ التقييم' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (action === 'logout' && req.method === 'POST') {
      await supabaseAdmin.from('tenant_sessions').delete().eq('session_token', sessionToken);
