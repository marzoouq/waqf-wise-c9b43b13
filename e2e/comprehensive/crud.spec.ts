...
test.describe('اختبارات CRUD الشاملة', () => {
  ...
  test('التصفح (Pagination)', async ({ page }) => {
    ...
    const pagination = page.locator(
      '[aria-label*="page"], button:has-text("التالي"), button:has-text("السابق"), .pagination'
    );
  ...
  test('عرض التفاصيل', async ({ page }) => {
    ...
    if (await tableRow.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const viewButton = tableRow.locator(
        'button:has-text("عرض"), a:has-text("عرض"), button[aria-label*="عرض"]'
      );
