...
test.describe('اختبارات الأزرار الشاملة', () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (await emailInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await page
        .waitForURL(/\/(dashboard|beneficiaries|admin)/, { timeout: 10000 })
        .catch(() => {});
    }
  });
  ...
  test('اختبار أزرار التصدير', async ({ page }) => {
    ...
    const exportButtons = page.locator(
      'button:has-text("تصدير"), button:has-text("Excel"), button:has-text("PDF")'
    );
  ...
  test('اختبار أزرار الحذف (تأكيد)', async ({ page }) => {
    ...
    const deleteButtons = page.locator(
      'button:has-text("حذف"), button[aria-label*="حذف"], button[aria-label*="delete"]'
    );
    ...
    if (count > 0) {
      ...
      if (await firstDelete.isVisible()) {
        ...
        const confirmDialog = page.locator(
          '[role="alertdialog"], [role="dialog"]:has-text("تأكيد"), [role="dialog"]:has-text("حذف")'
        );
  ...
  test('اختبار أزرار الطباعة', async ({ page }) => {
    ...
    const printButtons = page.locator(
      'button:has-text("طباعة"), button[aria-label*="طباعة"], button[aria-label*="print"]'
    );
