...
test.describe('رحلة المستفيد الكاملة', () => {
  ...
  test('الخطوة 3: تقديم طلب مساعدة', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("طلب جديد"), button:has-text("إضافة"), button:has-text("تقديم")'
    );
  ...
  test('الخطوة 4: رفع مرفق', async ({ page }) => {
    ...
    if (await fileInput.first().isVisible()) {
      ...
      const uploadButton = page.locator(
        'button:has-text("رفع"), button:has-text("إرفاق"), [data-testid="upload"]'
      );
  ...
  test('الخطوة 6: عرض كشف الحساب', async ({ page }) => {
    ...
    const hasStatementContent =
      content.includes('كشف') ||
  ...
  test('الخطوة 7: تصدير كشف الحساب', async ({ page }) => {
    ...
    const exportButton = page.locator(
      'button:has-text("تصدير"), button:has-text("PDF"), button:has-text("طباعة"), [data-testid="export"]'
    );
  ...
  test('الخطوة 9: عرض الإشعارات', async ({ page }) => {
    ...
    const hasNotificationsContent =
      content.includes('إشعار') || content.includes('notification') || content.length > 500;
  ...
  test('الخطوة 10: تسجيل الخروج', async ({ page }) => {
    ...
    const logoutButton = page.locator(
      'button:has-text("خروج"), button:has-text("تسجيل الخروج"), [data-testid="logout"]'
    );
