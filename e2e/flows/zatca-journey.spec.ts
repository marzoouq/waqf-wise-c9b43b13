...
test.describe('رحلة الزكاة ZATCA', () => {
  ...
  test('الخطوة 3: إنشاء فاتورة ضريبية', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("فاتورة"), button:has-text("إضافة"), button:has-text("جديد")'
    );
    ...
    if (await addButton.first().isVisible()) {
      ...
      const taxInvoiceOption = page.locator(
        'label:has-text("ضريبية"), input[value="tax"], [data-type="tax"]'
      );
  ...
  test('الخطوة 4: التحقق من البيانات', async ({ page }) => {
    ...
    const content = await page.content();
    const hasZatcaContent =
      content.includes('ZATCA') ||
  ...
  test('الخطوة 5: تشفير الفاتورة', async ({ page }) => {
    ...
    const encryptButton = page.locator(
      'button:has-text("تشفير"), button:has-text("توقيع"), [data-action="encrypt"]'
    );
  ...
  test('الخطوة 6: إرسال للهيئة', async ({ page }) => {
    ...
    const submitButton = page.locator(
      'button:has-text("إرسال للهيئة"), button:has-text("ZATCA"), button:has-text("submit")'
    );
  ...
  test('الخطوة 8: عرض حالة الإرسال', async ({ page }) => {
    ...
    if (await invoiceTable.first().isVisible()) {
      ...
      const statusColumn = page.locator(
        'th:has-text("حالة"), th:has-text("ZATCA"), th:has-text("status")'
      );
...
test.describe('اختبار Edge Function ZATCA', () => {
  test('التحقق من وجود وظيفة zatca-submit', async ({ page, request }) => {
    ...
    const response = await request
      .post(`${process.env.SUPABASE_URL || ''}/functions/v1/zatca-submit`, {
        headers: {
