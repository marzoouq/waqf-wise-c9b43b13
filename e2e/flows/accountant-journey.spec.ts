...
test.describe('رحلة المحاسب الكاملة', () => {
  ...
  test('الخطوة 3: إنشاء قيد يومي', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("قيد جديد"), button:has-text("إضافة قيد"), button:has-text("إنشاء")'
    );
  ...
  test('الخطوة 5: التحقق من توازن القيد', async ({ page }) => {
    ...
    const balanceIndicator = page.locator(
      'text=متوازن, text=balanced, .balance-ok, [data-balanced="true"]'
    );
  ...
  test('الخطوة 6: ترحيل القيد', async ({ page }) => {
    ...
    const postButton = page.locator(
      'button:has-text("ترحيل"), button:has-text("نشر"), button:has-text("post")'
    );
  ...
  test('الخطوة 7: دفتر الأستاذ', async ({ page }) => {
    ...
    const ledgerTab = page.locator(
      'button:has-text("الأستاذ"), a:has-text("الأستاذ"), [data-tab="ledger"]'
    );
  ...
  test('الخطوة 8: إنشاء فاتورة', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("فاتورة جديدة"), button:has-text("إضافة"), button:has-text("إنشاء")'
    );
  ...
  test('الخطوة 9: تسجيل دفعة', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("دفعة جديدة"), button:has-text("تسجيل دفعة"), button:has-text("إضافة")'
    );
  ...
  test('الخطوة 10: القيود الآلية', async ({ page }) => {
    ...
    const autoTab = page.locator(
      'button:has-text("آلي"), button:has-text("تلقائي"), [data-tab="auto"]'
    );
  ...
  test('الخطوة 11: ميزان المراجعة', async ({ page }) => {
    ...
    const trialBalanceTab = page.locator(
      'button:has-text("ميزان"), button:has-text("المراجعة"), [data-tab="trial-balance"]'
    );
